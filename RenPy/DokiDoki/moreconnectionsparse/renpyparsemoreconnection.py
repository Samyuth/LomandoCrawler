        ...
        for line in lines:
            ...
            if re.match(r"label (\w+):", line):
                ...
                if prev_label is not None:
                    self.nodes[prev_label]["connections"].append(label)
                prev_label = label
            elif re.match(r"jump (\w+)", line):
                ...
                if self.call_stack:
                    return_label = self.call_stack.pop()
                    if label in self.nodes:
                        self.nodes[label]["connections"].append(return_label)
            elif re.match(r"menu:", line):
                menu_start_index = next((i for i, ln in enumerate(lines) if 'menu:' in ln), None)
                if menu_start_index is None:
                    continue
